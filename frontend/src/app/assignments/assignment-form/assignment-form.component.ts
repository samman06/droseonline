  ...
  template: `
    <div class="container mx-auto px-4 py-6 max-w-5xl space-y-6">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div>
            <h1 class="text-3xl font-bold bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent">
              {{ (isEditMode ? 'assignments.editAssignment' : 'assignments.createAssignment') | translate }}
            </h1>
            <p class="text-gray-600 mt-1">{{ (isEditMode ? 'assignments.updateAssignmentDesc' : 'assignments.createAssignmentDesc') | translate }}</p>
          </div>
        </div>
      </div>
      ...
      <div *ngIf="loading" class="flex flex-col items-center justify-center py-16">
        ...
        <p class="mt-4 text-gray-600 font-medium">{{ 'common.loading' | translate }}</p>
      </div>
      ...
      <form *ngIf="!loading" [formGroup]="assignmentForm" (ngSubmit)="onSubmit()" class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200">
          <div class="bg-gradient-to-r from-indigo-50 to-purple-50 border-b border-gray-200 p-5 rounded-t-xl">
            <div class="flex items-center">
              ...
              <h2 class="text-xl font-bold text-gray-900">{{ 'assignments.basicInformation' | translate }}</h2>
            </div>
          </div>
          <div class="p-6">
          ...
          <div class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ 'assignments.title' | translate }} <span class="text-red-500">*</span>
                </label>
                <input 
                  type="text" 
                  formControlName="title"
                  class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                  [placeholder]="'assignments.enterTitle' | translate">
                <div *ngIf="assignmentForm.get('title')?.invalid && assignmentForm.get('title')?.touched" class="text-red-500 text-sm mt-1">
                  {{ 'assignments.titleRequired' | translate }}
                </div>
              </div>

              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ 'assignments.code' | translate }} <span class="text-xs text-gray-500">({{ 'assignments.autoGenerated' | translate }})</span>
                </label>
                <input 
                  type="text" 
                  formControlName="code"
                  class="w-full rounded-lg border-gray-300 bg-gray-100 cursor-not-allowed shadow-sm"
                  [placeholder]="'assignments.codeExample' | translate"
                  [disabled]="true">
                <p class="text-xs text-gray-500 mt-1">{{ 'assignments.codeAutoGenDesc' | translate }}</p>
              </div>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">
                {{ 'assignments.description' | translate }} <span class="text-red-500">*</span>
              </label>
              <textarea 
                formControlName="description"
                rows="4"
                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                [placeholder]="'assignments.enterDescription' | translate"></textarea>
              <div *ngIf="assignmentForm.get('description')?.invalid && assignmentForm.get('description')?.touched" class="text-red-500 text-sm mt-1">
                {{ 'assignments.descriptionRequired' | translate }}
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ 'assignments.type' | translate }} <span class="text-red-500">*</span>
                </label>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ 'assignments.groups' | translate }} <span class="text-red-500">*</span>
                </label>
                ...
                <div class="relative">
                  <button
                    ...
                    <span class="text-gray-700">
                      <span *ngIf="selectedGroups.length === 0" class="text-gray-400">{{ 'assignments.selectGroups' | translate }}</span>
                      <span *ngIf="selectedGroups.length > 0">{{ 'assignments.groupsSelected' | translate: {count: selectedGroups.length} }}</span>
                    </span>
                  ...
                  <div *ngIf="showGroupsDropdown" class="absolute z-50 w-full mt-2 bg-white border-2 border-gray-200 rounded-lg shadow-xl max-h-64 overflow-y-auto">
                    <div *ngIf="groups.length === 0" class="px-4 py-3 text-sm text-gray-500 text-center">
                      {{ 'assignments.noGroupsAvailable' | translate }}
                    </div>
                ...
                <p class="text-xs text-gray-500 mt-2">
                  ...
                  {{ 'assignments.groupsHint' | translate }}
                </p>
                ...
                <div *ngIf="assignmentForm.get('groups')?.invalid && assignmentForm.get('groups')?.touched" class="flex items-center text-red-600 text-sm mt-2">
                  ...
                  {{ 'assignments.groupRequired' | translate }}
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200">
          <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-200 p-5 rounded-t-xl">
            <div class="flex items-center">
              ...
              <h2 class="text-xl font-bold text-gray-900">{{ 'assignments.scheduleGrading' | translate }}</h2>
            </div>
          </div>
          <div class="p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">
                {{ 'assignments.dueDate' | translate }} <span class="text-red-500">*</span>
              </label>
              <input 
                type="datetime-local" 
              ...
              <div *ngIf="assignmentForm.get('dueDate')?.invalid && assignmentForm.get('dueDate')?.touched" class="text-red-500 text-sm mt-1">
                {{ 'assignments.dueDateRequired' | translate }}
              </div>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">
                {{ 'assignments.maxPoints' | translate }} <span class="text-red-500">*</span>
              </label>
              <input 
                type="number" 
              ...
              <div *ngIf="assignmentForm.get('maxPoints')?.invalid && assignmentForm.get('maxPoints')?.touched" class="text-red-500 text-sm mt-1">
                {{ 'assignments.maxPointsRequired' | translate }}
              </div>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">
                {{ 'assignments.weightage' | translate }} <span class="text-red-500">*</span>
              </label>
              <input 
                type="number" 
              ...
              <div *ngIf="assignmentForm.get('weightage')?.invalid && assignmentForm.get('weightage')?.touched" class="text-red-500 text-sm mt-1">
                {{ 'assignments.weightageRequired' | translate }}
              </div>
              <p class="text-xs text-gray-500 mt-1">{{ 'assignments.weightageDesc' | translate }}</p>
            </div>
          </div>
          </div>
        </div>
        ...
        <div *ngIf="assignmentForm.get('type')?.value === 'quiz'" class="bg-white rounded-xl shadow-lg border border-gray-200">
          <div class="bg-gradient-to-r from-blue-50 to-cyan-50 border-b border-gray-200 p-5 rounded-t-xl">
            <div class="flex items-center">
              ...
              <h2 class="text-xl font-bold text-gray-900">{{ 'assignments.quizSettings' | translate }}</h2>
            </div>
          </div>
          <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ 'assignments.timeLimit' | translate }}
                </label>
                <input 
                  type="number" 
                  ...
                  class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                  placeholder="60">
                <p class="text-xs text-gray-500 mt-1">{{ 'assignments.noTimeLimitHint' | translate }}</p>
              </div>

              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ 'assignments.resultsVisibility' | translate }}
                </label>
                <select formControlName="resultsVisibility" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                  <option value="immediate">{{ 'assignments.immediate' | translate }}</option>
                  <option value="after_deadline">{{ 'assignments.afterDeadline' | translate }}</option>
                  <option value="manual">{{ 'assignments.manual' | translate }}</option>
                  <option value="never">{{ 'assignments.never' | translate }}</option>
                </select>
              </div>
            </div>
            ...
            <div class="flex items-center space-x-6 mb-4">
              <label class="flex items-center">
                ...
                <span class="ml-2 text-sm text-gray-700">{{ 'assignments.shuffleQuestions' | translate }}</span>
              </label>
              <label class="flex items-center">
                ...
                <span class="ml-2 text-sm text-gray-700">{{ 'assignments.shuffleOptions' | translate }}</span>
              </label>
            </div>
          </div>
        </div>
          ...
          <div class="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-gray-200 p-5 rounded-t-xl">
            <div class="flex items-center justify-between">
              <div class="flex items-center">
                ...
                <div>
                  <h2 class="text-xl font-bold text-gray-900">{{ 'assignments.questions' | translate }}</h2>
                  <p class="text-sm text-gray-600">{{ 'assignments.questionsTotal' | translate: {count: questions.length, points: calculateTotalPoints()} }}</p>
                </div>
              </div>
              <button 
                type="button" 
                ...
                {{ 'assignments.addQuestion' | translate }}
              </button>
            </div>
          </div>
          <div class="p-6">
            <div formArrayName="questions" class="space-y-6">
              <div *ngFor="let question of questions.controls; let i = index" [formGroupName]="i" 
                ...
                <div class="flex items-center justify-between mb-4">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <h3 class="text-lg font-semibold text-gray-900">{{ 'assignments.question' | translate }} {{ i + 1 }}</h3>
                      <p class="text-sm text-gray-500">{{ 'assignments.points' | translate }}: {{ question.get('points')?.value || 1 }}</p>
                    </div>
                  </div>
                  <button 
                    type="button" 
                ...
                <div class="mb-4">
                  <label class="block text-sm font-medium text-gray-700 mb-2">
                    {{ 'assignments.question' | translate }} <span class="text-red-500">*</span>
                  </label>
                  <textarea 
                    formControlName="question"
                    rows="3"
                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500"
                    [placeholder]="'assignments.enterQuestion' | translate"></textarea>
                  <div *ngIf="question.get('question')?.invalid && question.get('question')?.touched" class="text-red-500 text-sm mt-1">
                    {{ 'assignments.questionRequired' | translate }}
                  </div>
                </div>
                ...
                <div class="mb-4">
                  <label class="block text-sm font-medium text-gray-700 mb-3">
                    {{ 'assignments.answerOptions' | translate }}
                  </label>
                  <div class="space-y-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                      {{ 'assignments.points' | translate }} <span class="text-red-500">*</span>
                    </label>
                    <input 
                      type="number" 
                  ...
                  <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                      {{ 'assignments.explanation' | translate }} ({{ 'common.optional' | translate }})
                    </label>
                    <input 
                      type="text" 
                      formControlName="explanation"
                      class="w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500"
                      [placeholder]="'assignments.explainAnswer' | translate">
                  </div>
                </div>
              </div>
              ...
              <div *ngIf="questions.length === 0" class="text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300">
                ...
                <p class="mt-2 text-sm text-gray-600">{{ 'assignments.noQuestionsYet' | translate }}</p>
                <button 
                  type="button" 
                  (click)="addQuestion()"
                  ...
                  {{ 'assignments.addFirstQuestion' | translate }}
                </button>
              </div>
            </div>
          </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200">
          <div class="bg-gradient-to-r from-purple-50 to-pink-50 border-b border-gray-200 p-5 rounded-t-xl">
            <div class="flex items-center justify-between">
              <div class="flex items-center">
                ...
                <h2 class="text-xl font-bold text-gray-900">{{ 'assignments.gradingRubric' | translate }}</h2>
              </div>
              <button 
                type="button" 
                ...
                {{ 'assignments.addCriterion' | translate }}
              </button>
            </div>
          </div>
          <div class="p-6">
            ...
            <div *ngFor="let criterion of rubric.controls; let i = index" [formGroupName]="i" 
              ...
              <div class="flex-1">
                <input 
                  ...
                  [placeholder]="'assignments.criterionName' | translate">
              </div>
              <div class="w-24">
                <input 
                  type="number" 
                  ...
                  min="0"
                  class="w-full rounded border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500"
                  [placeholder]="'assignments.points' | translate">
              </div>
              <button 
                type="button" 
          ...
          <div *ngIf="rubric.length === 0" class="text-center py-6 text-gray-500 text-sm">
            {{ 'assignments.noRubricCriteria' | translate }}
          </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200">
          <div class="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-gray-200 p-5 rounded-t-xl">
            <div class="flex items-center">
              ...
              <h2 class="text-xl font-bold text-gray-900">{{ 'assignments.submissionSettings' | translate }}</h2>
            </div>
          </div>
          <div class="p-6">
          ...
          <div class="space-y-3">
            <label class="flex items-center">
              ...
              <span class="ml-2 text-sm text-gray-700">{{ 'assignments.allowLateSubmissions' | translate }}</span>
            </label>

            <div *ngIf="assignmentForm.get('allowLateSubmissions')?.value" class="ml-6 mt-2">
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ 'assignments.latePenalty' | translate }}</label>
              <input 
                type="number" 
            ...
            <label class="flex items-center">
              <input type="checkbox" formControlName="requireFileUpload" 
                     class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
              <span class="ml-2 text-sm text-gray-700">{{ 'assignments.requireFileUpload' | translate }}</span>
            </label>

            <label class="flex items-center">
              <input type="checkbox" formControlName="allowMultipleSubmissions" 
                     class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
              <span class="ml-2 text-sm text-gray-700">{{ 'assignments.allowMultipleSubmissions' | translate }}</span>
            </label>
          </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200">
          <div class="bg-gradient-to-r from-orange-50 to-yellow-50 border-b border-gray-200 p-5 rounded-t-xl">
            <div class="flex items-center">
              ...
              <h2 class="text-xl font-bold text-gray-900">{{ 'assignments.additionalInstructions' | translate }}</h2>
            </div>
          </div>
          <div class="p-6">
            <textarea 
              ...
              [placeholder]="'assignments.instructionsPlaceholder' | translate"></textarea>
            <p class="text-xs text-gray-500 mt-2">
              <svg class="w-3 h-3 inline mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
              </svg>
              {{ 'assignments.instructionsHint' | translate }}
            </p>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
            <p class="text-sm text-gray-600">
              ...
              {{ (isEditMode ? 'assignments.updateDetails' : 'assignments.createAndPublish') | translate }}
            </p>
            <div class="flex gap-3">
              ...
              <button 
                ...
                {{ (saving ? 'common.saving' : (isEditMode ? 'assignments.updateAssignment' : 'assignments.publishAssignment')) | translate }}
              </button>
            </div>
          </div>
        </div>
...
export class AssignmentFormComponent implements OnInit {
  ...
  loadGroups(): void {
    ...
    groupsCall.subscribe({
          ...
          if (preSelectedGroupData && !this.isEditMode) {
            try {
              ...
              if (group) {
                ...
                this.toastService.success(this.translate.instant('assignments.groupPreSelected', { name: preSelectedGroup.name }));
              }
      ...
      error: (error) => {
        ...
        this.toastService.error(this.translate.instant('assignments.failedToLoadGroups'));
      }
    });
  }
      ...
      next: (response) => {
        if (response.success) {
          this.toastService.success(
            this.translate.instant(this.isEditMode ? 'assignments.assignmentUpdated' : 'assignments.assignmentPublished')
          );
          this.router.navigate(['/dashboard/assignments']);
        }
        this.saving = false;
  ...
  saveDraft(): void {
    ...
    this.assignmentService.createAssignment(formData).subscribe({
      next: (response) => {
        if (response.success) {
          this.toastService.success(this.translate.instant('assignments.assignmentSavedDraft'));
          this.router.navigate(['/dashboard/assignments']);
        }
        this.saving = false;
      },
