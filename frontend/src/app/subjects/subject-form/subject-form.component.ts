    ...
    <div class="max-w-3xl mx-auto p-6">
      ...
      <form [formGroup]="form" (ngSubmit)="submit()" class="bg-white rounded-xl shadow-sm border border-gray-200">
        <div class="px-8 py-6 border-b border-gray-200">
          <h2 class="text-xl font-semibold text-gray-900">{{ 'subjects.subjectInformation' | translate }}</h2>
        </div>

        <div class="p-8 space-y-6">
          <div>
            <label class="form-label">{{ 'subjects.name' | translate }}</label>
            <input class="form-input" formControlName="name" [placeholder]="'subjects.namePlaceholder' | translate" />
          </div>
          <div>
            <label class="form-label">{{ 'subjects.code' | translate }} <span class="text-xs text-gray-500">({{ 'subjects.autoGenerated' | translate }})</span></label>
            <input class="form-input bg-gray-100 cursor-not-allowed" formControlName="code" [placeholder]="'subjects.codePlaceholder' | translate" [disabled]="true" />
            <p class="text-xs text-gray-500 mt-1">{{ 'subjects.codeAutoGenerateHint' | translate }}</p>
          </div>
        ...
        <div class="flex justify-end space-x-3 px-8 py-6 bg-gray-50 border-t border-gray-200">
          <button type="button" (click)="cancel.emit()" class="btn-secondary">{{ 'common.cancel' | translate }}</button>
          <button type="submit" [disabled]="form.invalid || submitting" class="btn-primary">
            {{ submitting ? ('common.saving' | translate) : submitText }}
          </button>
        </div>
      </form>
    </div>
