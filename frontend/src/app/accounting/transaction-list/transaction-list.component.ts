        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
              ...
              <div>
                <h1 class="text-3xl font-bold text-gray-900">{{ 'accounting.transactionList.title' | translate }}</h1>
                <p class="mt-1 text-sm text-gray-500">{{ 'accounting.transactionList.subtitle' | translate }}</p>
              </div>
            </div>
            <button (click)="addNewTransaction()" 
              ...
              {{ 'accounting.transactionList.addTransaction' | translate }}
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ 'accounting.transactionList.type' | translate }}</label>
              <select [(ngModel)]="filters.type" (ngModelChange)="onFiltersChange()" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                <option value="">{{ 'accounting.transactionList.allTypes' | translate }}</option>
                <option value="income">{{ 'accounting.transactionList.income' | translate }}</option>
                <option value="expense">{{ 'accounting.transactionList.expense' | translate }}</option>
              </select>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ 'accounting.transactionList.category' | translate }}</label>
              <select [(ngModel)]="filters.category" (ngModelChange)="onFiltersChange()" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                <option value="">{{ 'accounting.transactionList.allCategories' | translate }}</option>
                <option *ngFor="let cat of getAllCategories()" [value]="cat.value">{{ cat.label }}</option>
              </select>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ 'accounting.transactionList.startDate' | translate }}</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ 'accounting.transactionList.endDate' | translate }}</label>
          ...
          <div class="mt-4 flex items-center justify-between">
            <div *ngIf="hasActiveFilters()" class="flex items-center space-x-2">
              <span class="text-sm text-gray-600">{{ 'accounting.transactionList.activeFilters' | translate }}:</span>
              <span *ngIf="filters.type" class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700">
                {{ filters.type === 'income' ? ('accounting.transactionList.income' | translate) : ('accounting.transactionList.expense' | translate) }}
              </span>
            ...
            <button *ngIf="hasActiveFilters()" (click)="clearFilters()" class="text-sm text-blue-600 hover:text-blue-800 font-medium">
              {{ 'accounting.transactionList.clearFilters' | translate }}
            </button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
          <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-green-100 text-sm font-medium">{{ 'accounting.transactionList.totalIncome' | translate }}</p>
          ...
          <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-red-100 text-sm font-medium">{{ 'accounting.transactionList.totalExpenses' | translate }}</p>
          ...
          <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-blue-100 text-sm font-medium">{{ 'accounting.transactionList.netBalance' | translate }}</p>
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                <tr>
                  <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">{{ 'accounting.transactionList.date' | translate }}</th>
                  <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">{{ 'accounting.transactionList.type' | translate }}</th>
                  <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">{{ 'accounting.transactionList.category' | translate }}</th>
                  <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">{{ 'accounting.transactionList.title' | translate }}</th>
                  <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">{{ 'accounting.transactionList.teacher' | translate }}</th>
                  <th class="px-6 py-4 text-right text-xs font-bold text-gray-700 uppercase tracking-wider">{{ 'accounting.transactionList.amount' | translate }}</th>
                  <th class="px-6 py-4 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">{{ 'accounting.transactionList.actions' | translate }}</th>
                </tr>
              </thead>
                ...
                <tr *ngFor="let transaction of transactions" class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold"
                      ...
                      {{ transaction.type === 'income' ? ('accounting.transactionList.income' | translate) : ('accounting.transactionList.expense' | translate) }}
                    </span>
                  </td>
                  ...
                  <td class="px-6 py-4 text-sm text-gray-900">
                    <div class="flex items-center gap-2">
                      ...
                      <div *ngIf="isAttendanceTransaction(transaction)" 
                        ...
                        <span class="font-medium">{{ 'accounting.transactionList.auto' | translate }}</span>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        ...
                        <div class="py-1">
                          ...
                          <ng-container *ngIf="isAttendanceTransaction(transaction)">
                            <button (click)="viewTransaction(transaction); closeDropdown()"
                              ...
                              {{ 'accounting.transactionList.view' | translate }}
                            </button>
                          </ng-container>
                          ...
                          <ng-container *ngIf="!isAttendanceTransaction(transaction)">
                            <button (click)="editTransaction(transaction); closeDropdown()"
                              ...
                              {{ 'accounting.transactionList.edit' | translate }}
                            </button>
                            ...
                            <button (click)="deleteTransaction(transaction); closeDropdown()"
                              ...
                              {{ 'accounting.transactionList.delete' | translate }}
                            </button>
                          </ng-container>
                        </div>
                      </div>
                ...
                <tr *ngIf="transactions.length === 0">
                  <td colspan="6" class="px-6 py-12 text-center">
                    <div class="flex flex-col items-center">
                      ...
                      <p class="text-gray-500 text-lg font-medium">{{ 'accounting.transactionList.noTransactions' | translate }}</p>
                      <p class="text-gray-400 text-sm mt-1">{{ 'accounting.transactionList.createFirstTransaction' | translate }}</p>
          ...
          <div *ngIf="pagination.pages > 1" class="bg-gray-50 px-4 py-3 flex items-center justify-between border-t border-gray-200">
            <div class="flex-1 flex justify-between sm:hidden">
              <button (click)="changePage(pagination.page - 1)" [disabled]="pagination.page === 1"
                ...
                {{ 'accounting.transactionList.previous' | translate }}
              </button>
              <button (click)="changePage(pagination.page + 1)" [disabled]="pagination.page === pagination.pages"
                ...
                {{ 'accounting.transactionList.next' | translate }}
              </button>
            </div>
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p class="text-sm text-gray-700">
                  {{ 'accounting.transactionList.showing' | translate }} <span class="font-medium">{{ (pagination.page - 1) * pagination.limit + 1 }}</span>
                  {{ 'accounting.transactionList.to' | translate }} <span class="font-medium">{{ Math.min(pagination.page * pagination.limit, pagination.total) }}</span>
                  {{ 'accounting.transactionList.of' | translate }} <span class="font-medium">{{ pagination.total }}</span> {{ 'accounting.transactionList.results' | translate }}
                </p>
              </div>
              <div>
...
export class TransactionListComponent implements OnInit {
  ...
  loadTransactions(): void {
    ...
    this.accountingService.getTransactions(params).subscribe({
      ...
      error: (error) => {
        ...
        this.toastService.error(this.translate.instant('accounting.transactionList.failedToLoad'));
        this.isLoading = false;
      }
    });
  }
  ...
  async deleteTransaction(transaction: any): Promise<void> {
    const confirmed = await this.confirmationService.confirm({
      title: this.translate.instant('accounting.transactionList.deleteTransaction'),
      message: this.translate.instant('accounting.transactionList.deleteConfirmMessage', { title: transaction.title }),
      confirmText: this.translate.instant('accounting.transactionList.yesDelete'),
    ...
    if (confirmed) {
      this.accountingService.deleteTransaction(transaction._id || transaction.id).subscribe({
        next: (response) => {
          if (response.success) {
            this.toastService.success(this.translate.instant('accounting.transactionList.deleteSuccess'));
            this.loadTransactions();
          }
        },
        error: (error) => {
          console.error('Error deleting transaction:', error);
          this.toastService.error(this.translate.instant('accounting.transactionList.deleteFailed'));
        }
      });
    }
  }
  ...
  getTeacherName(teacher: any): string {
    if (!teacher) return this.translate.instant('common.unknown');
    if (typeof teacher === 'string') return this.translate.instant('common.teacher');
    return teacher.fullName || `${teacher.firstName || ''} ${teacher.lastName || ''}`.trim() || teacher.email || this.translate.instant('common.unknown');
  }
