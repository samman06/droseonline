...
@Component({
  ...
  template: `
    <div class="container mx-auto px-4 py-6">
      ...
      <!-- Table View -->
      <div *ngIf="!loading && announcements.length > 0 && viewMode === 'table'" class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gradient-to-r from-blue-50 to-indigo-50">
              <tr>
                ...
                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                  {{ 'announcements.type' | translate }}
                </th>
                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                  {{ 'announcements.priority' | translate }}
                </th>
                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                  {{ 'announcements.publishedDate' | translate }}
                </th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr *ngFor="let announcement of announcements" 
                  ...
                  [class.bg-yellow-50]="announcement.isPinned">
                <!-- Title -->
                <td class="px-6 py-4">
                  <div class="flex items-center">
                    ...
                    <div class="ml-4">
                      <div class="flex items-center gap-2">
                        <a [routerLink]="['/dashboard/announcements', announcement._id]" 
                           class="text-sm font-bold text-gray-900 hover:text-blue-600 transition-colors">
                          {{ announcement.title }}
                        </a>
                        <svg *ngIf="announcement.isPinned" class="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                        </svg>
                      </div>
                      <div class="text-xs text-gray-500">{{ announcement.author?.fullName || 'System' }}</div>
                    </div>
                  </div>
                </td>
                ...
                <!-- Published Date -->
                <td class="px-6 py-4">
                  <div class="text-sm text-gray-700">{{ formatDate(announcement.publishAt) }}</div>
                </td>
                ...
                <td class="px-6 py-4">
                  <div class="flex items-center justify-center gap-2">
                    <button 
                      [routerLink]="['/dashboard/announcements', announcement._id]"
                      ...
                      {{ 'announcements.view' | translate }}
                    </button>
                    <button 
                      *ngIf="canEdit(announcement)"
      ...
      <!-- Card View (Announcements List) -->
      <div *ngIf="!loading && announcements.length > 0 && viewMode === 'card'" class="space-y-4">
