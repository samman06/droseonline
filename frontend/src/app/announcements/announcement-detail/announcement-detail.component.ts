...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, FormsModule, TranslateModule, AvatarComponent],
  template: `
    <div class="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-indigo-50/20 py-8 px-4 sm:px-6 lg:px-8">
      <div class="max-w-5xl mx-auto">
        ...
        <div *ngIf="!loading && announcement" class="space-y-6">
          ...
          <div class="bg-white rounded-xl shadow-lg p-8">
            <div class="flex items-start justify-between mb-6">
              <div class="flex-1">
                ...
                <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                  ...
                  <span class="flex items-center gap-1">
                    ...
                    {{ announcement.stats?.views || 0 }} {{ 'announcements.views' | translate }}
                  </span>
                  <span *ngIf="announcement.expiresAt" class="flex items-center gap-1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    {{ 'announcements.expires' | translate }} {{ formatDate(announcement.expiresAt) }}
                  </span>
                </div>
              </div>
              ...
              <div class="flex gap-2 ml-4">
                <button 
                  ...
                  [title]="'announcements.backToList' | translate">
            ...
            <div *ngIf="announcement.type === 'event' && announcement.eventDetails" 
              ...
              <h3 class="text-lg font-semibold text-blue-900 mb-4 flex items-center gap-2">
                ...
                {{ 'announcements.eventDetails' | translate }}
              </h3>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div *ngIf="announcement.eventDetails.eventDate">
                  <span class="text-gray-600">{{ 'announcements.date' | translate }}:</span>
                ...
                <div *ngIf="announcement.eventDetails.location">
                  <span class="text-gray-600">{{ 'announcements.location' | translate }}:</span>
                ...
                <div *ngIf="announcement.eventDetails.startTime">
                  <span class="text-gray-600">{{ 'announcements.time' | translate }}:</span>
                ...
                <div *ngIf="announcement.eventDetails.maxParticipants">
                  <span class="text-gray-600">{{ 'announcements.maxParticipants' | translate }}:</span>
            ...
            <div class="flex items-center gap-6 pt-6 border-t border-gray-200">
              <button 
                ...
                <span class="text-sm">{{ isLiked ? ('announcements.liked' | translate) : ('announcements.like' | translate) }}</span>
              </button>
              <div class="flex items-center gap-2 text-gray-600">
                ...
                <span class="text-sm">{{ 'announcements.comments' | translate }}</span>
              </div>
            </div>
          </div>
          ...
          <!-- Comments Section -->
          <div class="bg-white rounded-xl shadow-lg p-8">
            <h3 class="text-2xl font-bold text-gray-900 mb-6">{{ 'announcements.comments' | translate }}</h3>
            ...
            <div *ngIf="announcement.allowComments === false" class="bg-gray-50 rounded-lg p-6 text-center">
              ...
              <p class="text-gray-600 font-medium">{{ 'announcements.commentsDisabled' | translate }}</p>
            </div>
            ...
            <div *ngIf="announcement.allowComments !== false" class="mb-8">
              <div class="flex gap-3">
                ...
                <div class="flex-1">
                  <textarea 
                    [(ngModel)]="newComment"
                    [placeholder]="'announcements.writeComment' | translate"
                    rows="3"
                  ...
                  <div class="flex justify-end mt-2">
                    <button 
                      ...
                      {{ isSubmittingComment ? ('announcements.posting' | translate) : ('announcements.postComment' | translate) }}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div *ngIf="announcement.allowComments !== false" class="space-y-6">
              <div *ngIf="!announcement.comments || announcement.comments.length === 0" 
                ...
                <p class="text-lg font-medium">{{ 'announcements.noComments' | translate }}</p>
                <p class="text-sm mt-1">{{ 'announcements.beFirstToComment' | translate }}</p>
              </div>
        ...
        <div *ngIf="!loading && error" class="bg-white rounded-xl shadow-lg p-12 text-center">
          ...
          <button 
            ...
            {{ 'announcements.backToAnnouncements' | translate }}
          </button>
        </div>
...
export class AnnouncementDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.error = this.translate.instant('announcements.invalidId');
      this.loading = false;
    }
  }
  ...
  loadAnnouncement(id: string): void {
    ...
    this.announcementService.getAnnouncement(id).subscribe({
      ...
      error: (error) => {
        ...
        this.error = error.error?.message || this.translate.instant('announcements.failedToLoad');
        this.loading = false;
    ...
    this.announcementService.toggleLike(this.announcement._id).subscribe({
      next: (response) => {
        if (response.success && response.data) {
          ...
          this.toastService.success(this.isLiked ? this.translate.instant('announcements.likeSuccess') : this.translate.instant('announcements.likeRemoved'));
        }
      },
      error: (error) => {
        console.error('Error toggling like:', error);
        this.toastService.error(this.translate.instant('announcements.failedToUpdateLike'));
      }
    });
  }
    ...
    this.announcementService.addComment(this.announcement._id, this.newComment.trim()).subscribe({
      next: (response) => {
        if (response.success && response.data) {
          this.announcement = (response.data as any).announcement || response.data;
          this.newComment = '';
          this.toastService.success(this.translate.instant('announcements.commentAddedSuccess'));
        }
        this.isSubmittingComment = false;
      },
      error: (error) => {
        console.error('Error adding comment:', error);
        this.toastService.error(this.translate.instant('announcements.failedToAddComment'));
        this.isSubmittingComment = false;
      }
    });
  }
  ...
  deleteAnnouncement(): void {
    ...
    this.confirmationService.confirm({
      title: this.translate.instant('announcements.deleteConfirmTitle'),
      message: this.translate.instant('announcements.deleteConfirmMessage'),
      ...
      if (confirmed && this.announcement?._id) {
        this.announcementService.deleteAnnouncement(this.announcement._id).subscribe({
          next: (response) => {
            if (response.success) {
              this.toastService.success(this.translate.instant('announcements.deleteSuccess'));
              this.router.navigate(['/dashboard/announcements']);
            }
          },
          error: (error) => {
            console.error('Error deleting announcement:', error);
            this.toastService.error(this.translate.instant('announcements.failedToDelete'));
          }
        });
      }
    });
