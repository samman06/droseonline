  ...
  template: `
    <div class="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div *ngIf="teacher && !isLoading" class="space-y-6">
        ...
        <div *ngIf="teacher.academicInfo?.subjects && teacher.academicInfo.subjects.length > 0" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div *ngFor="let subject of teacher.academicInfo.subjects" 
                   class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg border-2 border-blue-200 p-4">
                <div class="flex items-start justify-between">
                  <div class="flex-1">
                    <h3 class="text-base font-bold text-gray-900 mb-1">{{ subject.name }}</h3>
                    <p class="text-xs text-gray-600 font-mono bg-white px-2 py-1 rounded inline-block">{{ subject.code }}</p>
                  </div>
                  <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                  </svg>
                </div>
                <div *ngIf="subject.credits || subject.type" class="mt-3 flex items-center space-x-3 text-xs text-gray-600">
                  <span *ngIf="subject.credits" class="inline-flex items-center">
        ...
        <!-- No Subjects Message -->
        <div *ngIf="!teacher.academicInfo?.subjects || teacher.academicInfo.subjects.length === 0" 
        ...
        <div *ngIf="teacher.academicInfo?.groups && teacher.academicInfo.groups.length > 0" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div *ngFor="let group of teacher.academicInfo.groups" 
                   class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg border-2 border-purple-200 p-4 hover:shadow-md transition-all duration-200 cursor-pointer"
                   [routerLink]="['/dashboard/groups', group._id || group.id]">
                <div class="flex items-start justify-between mb-3">
                  <div class="flex-1">
                    <h3 class="text-base font-bold text-gray-900 mb-1">{{ group.name }}</h3>
                    <p class="text-xs text-gray-600 font-mono bg-white px-2 py-1 rounded inline-block">{{ group.code }}</p>
                  </div>
                ...
                <div class="space-y-2 text-xs text-gray-600">
                  <div *ngIf="group.gradeLevel" class="flex items-center">
                    <svg class="w-4 h-4 mr-2 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    </svg>
                    <span class="font-medium">{{ group.gradeLevel }}</span>
                  </div>
                  <div *ngIf="group.students && group.students.length > 0" class="flex items-center">
                    <svg class="w-4 h-4 mr-2 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- No Groups Message -->
        <div *ngIf="!teacher.academicInfo?.groups || teacher.academicInfo.groups.length === 0" 
             class="bg-amber-50 border border-amber-200 rounded-xl p-6">
          <div class="flex items-center">
            <svg class="w-6 h-6 text-amber-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
